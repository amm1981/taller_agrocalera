...
export function DashboardPage() {
  ...
  const dashboard = useQuery({
    queryKey: ['inicio-horometros-dashboard', filters],
  ...
  return (
    <AppLayout title="Inicio" description="Estado operativo de horómetros">
      <section className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          ...
          <div className="flex items-end">
            <Button className="w-full md:w-auto" variant="secondary" onClick={() => void dashboard.refetch()} disabled={dashboard.isFetching}>
      ...
      {dashboard.isError ? (
        <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900">
          No se pudo actualizar la información de horómetros.
        </div>
      ) : null}
    </AppLayout>
  )
